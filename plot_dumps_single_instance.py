        ...
        for n_samples in [2000, 5000, 10000]:
            for n_dim in [1000, 2000, 5000]:
                for sigma_q in [3, 10, 20]:
                    ...
                    plt.subplot(1, 4, 4)
                    plt.title(
                        "L2-norm Errors of First Stage Models.\n n_samples: {}, n_dim: {}".format(n_samples, n_dim)
                    )
                    ...
                    plt.savefig(
                        os.path.join(
                            opts.output_dir,
                            "comparison_of_each_method_n_samples_{}_n_dim_{}_n_exp_{}_sigma_q_{}.png".format(
                                n_samples, n_dim, n_experiments, sigma_q
                            ),
                        ),
                        dpi=300,
                    )
                    plt.savefig(
                        os.path.join(
                            opts.output_dir,
                            "comparison_of_each_method_n_samples_{}_n_dim_{}_n_exp_{}_sigma_q_{}.pdf".format(
                                n_samples, n_dim, n_experiments, sigma_q
                            ),
                        ),
                        dpi=300,
